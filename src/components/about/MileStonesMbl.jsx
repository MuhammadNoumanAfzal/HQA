    ...
    <div className="px-4 py-8 font-serif relative w-full max-w-md mx-auto bg-white">
      ...
      >
        {milestones.map((milestone, index) => {
          const isOddCard = index % 2 === 0; // 0, 2, 4, 6, 8 are odd positions
          const isEvenCard = index % 2 === 1; // 1, 3, 5, 7 are even positions

          return (
            <div key={index} className="flex-shrink-0 w-full snap-center px-2">
              {/* Milestone Box - Fixed height for all cards */}
              <div
                className={`border rounded-2xl shadow-2xl p-6 ${
                  isOddCard ? "bg-white" : "bg-[#E8F4FF]"
                } transition-colors duration-300 h-[780px] flex flex-col`}
              >
                {/* ODD CARDS (1,3,5,7,9): Image → Circle → Image → UL */}
                {isOddCard && (
                  <>
                    {/* Top Image */}
                    ...
                    {/* Gray Circle with Year */}
                    <div className="flex justify-center h-32">
                      <div className="w-32 h-32 bg-gray-200 rounded-full flex items-center justify-center">
                        <p className="text-lg font-semibold text-center text-[#00285E]">
                          {milestone.year}
                        </p>
                      </div>
                    </div>
                    ...
                    {/* Bottom Image */}
                    <div className="flex justify-center mt-8 h-26">
                      <img
                    ...
                    {/* Milestone Points - No scrollbar */}
                    <ul className="mt-6 space-y-3 text-lg list-none pl-0 flex-1 overflow-hidden">
                ...
                {/* EVEN CARDS (2,4,6,8): Circle → Image → Image → UL */}
                {isEvenCard && (
                  <>
                    {/* Top Circle */}
